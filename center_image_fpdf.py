...
def center_image(pdf_obj, img_path, container_width, container_height, container_x=0, container_y=0, margin=0):
    ...
    if image_ratio == container_ratio:      # Container com a mesma proporção da imagem
        pdf_obj.image(name=img_path, w=container_width - margin, h=container_height - margin, x=container_x + margin, y=container_y + margin)
        return {
            'width': container_width - margin,
            'height': container_height - margin,
            'x': container_x + margin,
            'y': container_y + margin
        }
    
    elif image_ratio > container_ratio:           # Imagem na horizontal e container na vertical
        new_width = container_width - margin
        new_height = new_width / image_ratio
        new_y = container_y + ((container_height - new_height) / 2)
        new_x = container_x + margin
        ...
        new_height = container_height - margin
        new_width = new_height * image_ratio
        new_x = container_x + ((container_width - new_width) / 2)
        new_y = container_y + margin
